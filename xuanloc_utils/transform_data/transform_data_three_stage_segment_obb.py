...
from ..common import create_folder, cal_custom_iou_poly, box2poly, poly2box, get_min_rect
...
def create_label_obb(label_path, label):
    f = open(label_path, 'w')
    lines = []
    for obj in label:
        line = str(obj[0])
...
def transform_data_three_stage_segment(input_path, output_stage1_path, output_stage2_path, output_stage3_path, stage1_c, stage2_c_list_map, stage3_c_list_map):
    ...
    output_imgs_stage3_path = os.path.join(output_stage3_path, 'images')
    output_labels_stage3_path = os.path.join(output_stage3_path, 'labels')
    ...
    create_folder(output_stage3_path)
    create_folder(output_imgs_stage3_path)
    create_folder(output_labels_stage3_path)
    ...
    for img_name in tqdm(img_names):
        ...
        label_path = os.path.join(input_labels_path, img_name[:-4] + '.json')
        ...
        stage3_objs = [obj for obj in label if obj['label'] in stage3_c_list_map.keys()]

        # ----------------------------------------------
        # save stage1
        output_img_stage1_path = os.path.join(output_imgs_stage1_path, img_name)
        output_label_stage1_path = os.path.join(output_labels_stage1_path, img_name[:-4] + '.txt')
        ...
        for i, stage1_obj in enumerate(stage1_objs):
            ...
            crop_img_name = img_name[:-4] + f'_{round(stage1_obj_box[0], 2)}_{round(stage1_obj_box[1], 2)}_{round(stage1_obj_box[2], 2)}_{round(stage1_obj_box[3], 2)}.jpg'
        ...
        for i, stage1_obj in enumerate(stage1_objs):
            ...
            for stage3_obj in stage3_objs:
                stage3_obj_poly = get_obj_poly(stage3_obj)
                if cal_custom_iou_poly(stage3_obj_poly, stage1_obj_poly) > IOU_TH:
                    inside_stage3_objs.append(stage3_obj)
            
            # save image
            stage1_obj_box = poly2box(stage1_obj_poly)
            crop_img = img[int(stage1_obj_box[1]):int(stage1_obj_box[3]), int(stage1_obj_box[0]):int(stage1_obj_box[2])]
            crop_img_name = img_name[:-4] + f'_{round(stage1_obj_box[0], 2)}_{round(stage1_obj_box[1], 2)}_{round(stage1_obj_box[2], 2)}_{round(stage1_obj_box[3], 2)}.jpg'
            cv2.imwrite(os.path.join(output_imgs_stage3_path, crop_img_name), crop_img)

            # scale label
            obb_label = []
            for inside_stage3_obj in inside_stage3_objs:
                stage3_obj_poly = get_obj_poly(inside_stage3_obj)
                
                # get min rect 
                p1, p2, p3, p4 = get_min_rect(stage3_obj_poly)
                
                # scale with size
                p1 = [p1[0] - stage1_obj_box[0], p1[1] - stage1_obj_box[1]]
                p1 = [p1[0] / crop_img.shape[1], p1[1] / crop_img.shape[0]]
                
                p2 = [p2[0] - stage1_obj_box[0], p2[1] - stage1_obj_box[1]]
                p2 = [p2[0] / crop_img.shape[1], p2[1] / crop_img.shape[0]]
                
                p3 = [p3[0] - stage1_obj_box[0], p3[1] - stage1_obj_box[1]]
                p3 = [p3[0] / crop_img.shape[1], p3[1] / crop_img.shape[0]]
                
                p4 = [p4[0] - stage1_obj_box[0], p4[1] - stage1_obj_box[1]]
                p4 = [p4[0] / crop_img.shape[1], p4[1] / crop_img.shape[0]]
                
                obj = [stage3_c_list_map[inside_stage3_obj['label']]] + [p1, p2, p3, p4]
                obb_label.append(obj)
            crop_label_name = crop_img_name[:-4] + '.txt'
            create_label_obb(os.path.join(output_labels_stage3_path, crop_label_name), obb_label)
...
if __name__ == '__main__':
    ...
    output_stage3_path = '/media/xuanloc/sandisk500G/deep_learning/denso_project/training_pipeline/data/anh_clock_stage3'

    stage1_c = 'clock'
    stage2_c_list_map = {'kim': 0, 'zone': 1}
    stage3_c_list_map = {'hour': 0, 'minute': 1, 'second': 2}

    transform_data_three_stage_segment(input_path, output_stage1_path, output_stage2_path, output_stage3_path, stage1_c, stage2_c_list_map, stage3_c_list_map)